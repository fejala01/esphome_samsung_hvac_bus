...
static int packet_fail = 0;  // Fehlerzähler für gescheiterte Pakete
        ...
        void process_nasa_packet(MessageTarget *target)
        {
            ...
            while (it != sent_packets.end()) {
                if (now - it->last_sent_time > 3000 && it->retry_count < 3) {
                    ...
                    packet_fail++;
                    if (id(packet_fail_count_sensor) != nullptr) {
                        id(packet_fail_count_sensor).publish_state(packet_fail)
